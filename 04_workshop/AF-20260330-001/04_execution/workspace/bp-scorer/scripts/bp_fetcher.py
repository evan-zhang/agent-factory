...
- 获取组织BP简单树
- 获取单个BP的目标+KR+举措完整详情
- 从举措的 taskUsers 中提取承接方（role="承接人"）
- 批量获取个人BP入口 groupId
- 获取完整BP任务树
"""
...
APP_KEY = os.environ.get("BP_APP_KEY", "TsFhRR7OywNULeHPqudePf85STc4EpHI")
...
def get_goal_and_key_result(bp_id: str, period_id: str) -> dict:
    """
    获取单个 BP 的完整详情：目标 + KR + 举措（含 taskUsers 承接人）。
    ...
    # 字段标准化映射
    bp = {
        "id": raw.get("bpId") or raw.get("id", bp_id),
        "name": strip_html(raw.get("objective", "")) or raw.get("name", ""),
        "objective": raw.get("objective", ""),
        "keyResults": [],
    }

    # keyResults 处理：清理 HTML
    key_results = raw.get("keyResults", []) or []
    for kr in key_results:
        bp["keyResults"].append({
            "krId": kr.get("krId", ""),
            "name": strip_html(kr.get("description", "")) or kr.get("name", ""),
            "description": kr.get("description", ""),
            "target": kr.get("target", ""),
            "actions": [],
        })

    # actions（举措）来自 keyResults[].actions
    for kr in key_results:
        actions = kr.get("actions", []) or []
        for action in actions:
            bp["keyResults"][-1]["actions"].append({
                "actionId": action.get("measureId", ""),
                "description": strip_html(action.get("description", "")),
                "taskUsers": action.get("taskUsers", []) or [],
            })
...
def get_undertakers_from_bp(bp_detail: dict) -> list[dict]:
    """
    从 BP 详情中提取所有承接方（taskUsers role="承接人"）。

    按举措粒度独立处理：返回结构为
      [{"actionId": str, "action_desc": str, "undertakers": [{"userId", "name", ...}]}]

    已知问题修复：
    - 承接人从 taskUsers（role="承接人"）中提取，而非 taskDepts
    - None 安全：所有 list.get() 改为 list.get() or []

    :param bp_detail: get_goal_and_key_result 返回的 BP 详情
    :return: 按举措分组的承接方列表
    """
    result = []
    key_results = bp_detail.get("keyResults", []) or []
    for kr in key_results:
        actions = kr.get("actions", []) or []
        for action in actions:
            # 从 taskUsers 中提取 role="承接人"
            undertakers = [
                u for u in action.get("taskUsers", []) or []
                if u.get("role") == "承接人"
            ]
            if undertakers:
                result.append({

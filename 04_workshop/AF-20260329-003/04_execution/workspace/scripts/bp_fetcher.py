...
def search_by_name(name: str, period_id: str = None) -> list[dict]:
    """
    按组织名称精确搜索，返回匹配的组织列表。

    每个元素结构：{"id": str, "name": str, "type": str, ...}
    若返回多条，调用方须提示用户选择，不可自动取第一条。

    :param name: 组织名称（完整或部分）
    :param period_id: BP周期ID（必需）
    :return: 匹配的组织列表
    """
    if period_id is None:
        raise ValueError("period_id is required for search_by_name")
    resp = _get("/bp/group/searchByName", params={"name": name, "periodId": period_id})
...
def get_all_periods_global() -> list[dict]:
    """
    获取全局BP周期列表（不需要groupId）。

    :return: 周期列表，每项含 {id, name, status, ...}
    """
    resp = _get("/bp/period/getAllPeriod")
    return resp.get("data", [])
...
def get_simple_tree(group_id: str, period_id: str) -> list[dict]:
    ...
    :return: BP 节点列表
    """
    # API 路径已修正：补全 /bp/task/v2/ 前缀（2026-03-29 验证）
    resp = _get("/bp/task/v2/getSimpleTree", params={"groupId": group_id, "periodId": period_id})
    return resp.get("data", [])
...
def get_goal_and_key_result(bp_id: str, period_id: str) -> dict:
    ...
    :return: BP 详情字典
    """
    # API 路径已修正：补全 /bp/task/v2/ 前缀，参数名为 id（非 bpId）（2026-03-29 验证）
    resp = _get("/bp/task/v2/getGoalAndKeyResult", params={"id": bp_id, "periodId": period_id})
    return resp.get("data", {})


def get_undertakers_from_bp(bp_detail: dict) -> list[dict]:
    """
    从 BP 详情中提取所有承接方（负责人 role == "承接人"）。

    按举措(action)粒度独立处理：返回结构为
      [{"measureId": str, "measure_desc": str, "undertakers": [{"id", "name", ...}]}]
    ...
    # 从 keyResults[].actions 中提取
    for kr in (bp_detail.get("keyResults", []) or []):
        for action in (kr.get("actions", []) or []):
            undertakers = []
            # 从 taskUsers 中提取 role == "承接人" 的负责人
            for tu in (action.get("taskUsers", []) or []):
                if tu.get("role") == "承接人":
                    for emp in (tu.get("empList", []) or []):
                        undertakers.append({
                            "id": emp.get("id", ""),
                            "name": emp.get("name", ""),
                        })
            if undertakers:
                result.append({
                    "measureId": action.get("id", ""),
                    "measure_desc": action.get("name", ""),
                    "undertakers": undertakers,
                })
    
    # 如果 action 层级没有，从顶层 taskUsers 提取
    if not result:
        undertakers = []
        for tu in (bp_detail.get("taskUsers", []) or []):
            if tu.get("role") == "承接人":
                for emp in (tu.get("empList", []) or []):
                    undertakers.append({
                        "id": emp.get("id", ""),
        ...
        if undertakers:
            result.append({
                "measureId": bp_detail.get("id", ""),
                "measure_desc": bp_detail.get("name", ""),
                "undertakers": undertakers,
            })
...
def get_acceptor_action_counts(bp_detail: dict) -> dict:
    """
    递归统计每个承接人在该BP中参与的action数量（包括所有层级）。

    递归遍历：
    - 当前节点本身的 taskUsers（承接人）
    - keyResults -> actions -> downTaskList -> keyResults -> actions -> ...
    - 所有层级的 role="承接人" 都会被统计
    ...
    def collect(node):
        # 收集当前节点的 taskUsers 承接人
        for tu in (node.get("taskUsers") or []):
            if tu.get("role") == "承接人":
                for emp in (tu.get("empList") or []):
                    name = emp.get("name", "")
                    if name:
            ...
            for tu in (action.get("taskUsers") or []):
                if tu.get("role") == "承接人":
                    for emp in (tu.get("empList") or []):
                        ...
                        if name:
                            counts[name] = counts.get(name, 0) + 1
            # 递归 action 的 downTaskList
            collect(action)
        
        # 递归 keyResults
        for kr in (node.get("keyResults") or []):
            collect(kr)
        
        # 递归 downTaskList（子目标）
        for sub in (node.get("downTaskList") or []):
            collect(sub)

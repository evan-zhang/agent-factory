...
URL_CHAT_LIST_BY_PAGE = f"{API_BASE}/open-api/ai-huiji/meetingChat/chatListByPage"
...
def detect_combine_state(meeting_chat_id: str):
    result = _call_api(
        URL_CHAT_LIST_BY_PAGE,
        {"meetingChatId": meeting_chat_id, "pageNum": 1, "pageSize": 1},
        timeout=30,
    )
    if result.get("resultCode") != 1:
        return None
    ...
    if not data:
        return None
    return data[0].get("combineState")
...
def run_pull_once(meeting_chat_id: str, gateway=None, name="", force=False, lock_ttl=LOCK_TTL_SECONDS) -> dict:
    ...
    try:
        ...
        try:
            ...
            combine_state = detect_combine_state(meeting_chat_id)
            if combine_state == 2:
            ...
            if combine_state == 3:
                ...
                manifest["stopped_at"] = now_iso()
                manifest["stopped_reason"] = "combine_state_failed"
                store.save_manifest(manifest)
                return {
                    "status": "failed",
                    "meeting_chat_id": meeting_chat_id,
                    "reason": manifest["stopped_reason"],
                }
            if combine_state in (0, 1):

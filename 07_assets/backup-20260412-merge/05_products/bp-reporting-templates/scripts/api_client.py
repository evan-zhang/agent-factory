...
class BPAPIClient:
    ...
    def batch_get_key_position_markdown(self, group_ids: List[str]) -> str:
        """API 4.17: 批量获取关键岗位详情（Markdown 格式）
        
        替代已废弃的 4.16 接口。返回 Markdown 字符串，包含分组信息、
        奖金系数、目标列表（目标ID+名称+权重）。不含关键成果和关键举措。
        
        Args:
            group_ids: 个人类型分组 ID 列表
        Returns:
            Markdown 格式字符串
        """
        url = f"{self.base_url}/bp/group/batchGetKeyPositionMarkdown"
        payload = [int(gid) for gid in group_ids]
        response = requests.post(url, json=payload, headers=self.headers, timeout=20)
        response.raise_for_status()
        result = response.json()
        if result.get("resultCode") != 1:
            raise RuntimeError(f"批量获取关键岗位失败: {result.get('resultMsg')}")

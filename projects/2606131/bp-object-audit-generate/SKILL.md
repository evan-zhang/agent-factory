...
> **Evan 落地 v0.1.2（PROJECT_ROOT = `/Users/evan/Documents/BP`）** — 原版来自康哲集团 joy 同事本地，已去 Codex 特化、引入 PROJECT_ROOT 机制，缺省源材料「集团及中心BP/」缺失待补。v0.1.2 修复 Reviewer 指出的 3 个中等问题：agent.yaml 补 triggers/version、SKILL.md 压到 79 行。
...
| Rule | Requirement |
|---|---|
| `one_object_at_a_time` | Process exactly one BP object per cycle unless the user explicitly asks for a batch summary. |
...
1. Identify the BP object and level: group objective, center objective, department BP, or key individual BP.
...
7. Ask one focused confirmation block at a time when needed (issue / risk / recommended option / 2-3 alternatives / ask user). See `object_templates.md` § 3.
...
11. If the user confirms, archive the BP object and update package status files per `references/output_package.md`.

## Embedded Minimum Rules (No-Tool Fallback)

1. **One object at a time.** Never generate two BP objects in one turn.

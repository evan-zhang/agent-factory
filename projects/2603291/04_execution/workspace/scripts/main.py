...
def cmd_run(args: argparse.Namespace) -> None:
    ...
    # Step 1: 获取全局周期列表（不需要groupId）
    logger.info("获取BP周期列表...")
    periods = get_all_periods_global()
    ...
    # Step 2: 搜索组织（需要periodId）
    logger.info("搜索组织: %s", org_name)
    orgs = search_by_name(org_name, period_id=period_id)
    if not orgs:
    ...
    # Step 5: Agent 评分模式
    agent_score_mode = getattr(args, 'agent_score', False)
    scores_json_path = Path(output_dir) / "bp-scores.json"

    if agent_score_mode:
        # 生成 bp-list.json 供 Agent 评分
        bp_list_for_agent = []
        for bp in bp_details:
            undertakers_info = get_undertakers_from_bp(bp)
            undertaker_names = []
            for item in undertakers_info:
                for person in item.get("undertakers", []):
                    name = person.get("name", "")
                    if name and name not in undertaker_names:
                        undertaker_names.append(name)
            
            bp_list_for_agent.append({
                "bp_id": bp.get("id", ""),
                "bp_name": bp.get("name", ""),
                "undertakers": undertaker_names,
                "kr_count": len(bp.get("keyResults", []) or []),
            })

        bp_list_data = {
            "org_name": org_name,
            "period_name": period_name,
            "bp_list": bp_list_for_agent,
        }
        bp_list_path = Path(output_dir) / "bp-list.json"
        with open(bp_list_path, "w", encoding="utf-8") as f:
            json.dump(bp_list_data, f, ensure_ascii=False, indent=2)
        
        logger.info("BP数据已生成: %s", bp_list_path)
        
        # 检查是否有 Agent 评分结果
        if scores_json_path.exists():
            logger.info("发现 Agent 评分结果: %s", scores_json_path)
            with open(scores_json_path, "r", encoding="utf-8") as f:
                score_result = json.load(f)
            logger.info("已加载 %d 个 BP 评分", len(score_result.get("scores", [])))
        else:
            print(f"\n📋 BP数据已准备完毕，请 Agent 读取 {bp_list_path} 进行评分")
            print(f"   Agent 评分后，将结果写入: {scores_json_path}")
            print(f"   格式: {{\"scores\": [{{\"bp_id\": \"...\", \"score\": 0-100, \"reason\": \"...\"}}]}}")
            print(f"   完成后重新运行: python3 scripts/main.py run --org \"{org_name}\" --period \"{period_name}\" --agent-score")
            sys.exit(0)
    else:
        # Step 5: 同级BP权重评分（使用 LLM API）
    ...
    for bp in bp_details:
        ...
        bp_id = bp.get("id", "") or bp.get("bpId", "")
        s = score_map.get(bp_id, {})
        
        # 提取 KR 列表用于报告展示（清理 HTML 标签）
        import re
        def strip_html(text):
            if not text:
                return ""
        ...
        key_results = []
        for kr in (bp.get("keyResults", []) or []):
            kr_name = strip_html(kr.get("name") or "")
            if kr_name:
                key_results.append(kr_name[:150])  # 限制长度
        
        node = {
            "id": bp_id,
            "name": bp.get("name", "") or bp.get("objective", "（无标题）"),
            "type": "bp",
            ...
            "org_path": org_name,
            "source_bp_ids": [bp_id],
            "key_results": key_results,  # 新增：KR 列表
        }

        # 处理承接方 - 按 action 数量分配分数
        from bp_fetcher import get_acceptor_action_counts
        action_counts = get_acceptor_action_counts(bp)  # {人名: action数}
        
        if action_counts:
            total_actions = sum(action_counts.values())
            bp_score = node["score"]
            
            # 为每个负责人创建子节点，按action数量分配分数
            for uname, action_count in action_counts.items():
                # 该负责人获得的分数 = BP总分 × (action数 / 总action数)
                person_score = round(bp_score * action_count / total_actions, 1)
                child = {
                    "id": f"{bp_id}-{uname}",
                    "name": uname,
                    "type": "person",
                    ...
                    "adjust_reason": None,
                    "confidence": None,
                    "reason": f"承接《{bp.get('objective', '')[:30]}...》({action_count}个action)",
                    "tags": [],
                    "children": [],
                    "parent_ratio": action_count / total_actions,
...
""",
    ...
    run_parser.add_argument("--agent-score", action="store_true", help="使用 Agent 评分模式（不调用外部 LLM API）")

...
def _call_llm(prompt: str, retry_count: int = 3) -> Optional[str]:
    ...
    url = f"{SCORER_API_BASE}/v1/messages"
    headers = {
...
def _format_bp_for_prompt(bp: dict, index: int) -> str:
    ...
    kr_text = "\n".join(
        f"  - KR{i+1}: {kr.get('name', '') or kr.get('description', '')}"
        ...
        for a in actions:
            ...
            for td in a.get("taskDepts", []) or []:
                if td.get("role") == "undertaker":
                    undertakers.extend(d.get("name", "") for d in td.get("deptList", []) or [])
            all_measures.append({
                "name": a.get("name", ""),
    ...
    if not all_measures:
        for m in bp.get("measures", []):
            undertakers = [d.get("name", "") for d in m.get("taskDepts", []) if d.get("role") == "undertaker"]
            all_measures.append({
    ...
    measure_text = "\n".join(
        f"  - 举措{i+1}: {m['name']}（承接方：{', '.join(m['undertakers']) or '无'}）"

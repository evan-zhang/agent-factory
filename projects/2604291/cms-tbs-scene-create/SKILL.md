...
2. Step1-4 编排：
   ...
   - Step3：读取 `references/step3/tbs-scene-validate.md`，校验通过后立即读取 `references/step3/scenario-parse.md` 生成并确认 `title + sceneBackground`
   - Step4.1：读取 `references/step3/tbs-knowledge-topic-generate.md`（仅在 `title + sceneBackground` 已确认后执行），基于“医生核心顾虑+代表目标”生成 2-4 条产品知识主题，回显给用户逐条确认/可修改
...
### 可选配置
- `--base-url`：TBS 后台地址，默认 `https://cwork-web-test.xgjktech.com.cn/tbs-admin`（可通过环境变量 `TBS_BASE_URL` 覆盖）

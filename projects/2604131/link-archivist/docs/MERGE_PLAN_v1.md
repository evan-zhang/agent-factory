...
**执行流程**：
1. 计算 md_path 的 SHA256，比对 `kb_cache.json`
2. 若 SHA256 未变 → 检查 entries.json 是否已有该 path → 有则直接返回（no-op）
3. 解析 frontmatter（parse_entry）→ 得到 entry
4. 写入 entries.json（更新或新增）
5. 增量刷新 entities-registry.json（对该 entry 涉及的实体追加文档引用）
6. 增量刷新 graph-data.json（同 entry 涉及的实体两两建立 related_to 边）
7. 增量刷新 embeddings.json（如果配置了语义搜索）
8. 更新 kb_cache.json（写入新 SHA256 + status=success）
9. 写入 build_stats.json

**失败处理**：
- frontmatter 缺失必填字段 → 记录到 `kb_cache.json` 的 failed 状态，下一次还会重试
- entries.json 写入失败 → 抛异常，调用方决定是否 rollback
- 部分更新成功（entries 更新了但 graph-data 没刷）→ 启动时检测并自愈

**并发安全**：
- 不支持并发。单进程串行写。
- 如果用户同时归档多个文件，archive_report.py 内部串行调用 update_single
...
- 存在 → 读出 watch_dirs[0]，如果与 link-archivist 的 archive_dir 一致 → 合并到 kb_index 节，删除旧文件
- 不一致 → 提示用户手动处理（警告但不强制）
- 不存在 → 直接用 default
...
- 读 `~/.openclaw/kb-graph-config.json` → 合并到 `~/.openclaw/link-archivist-config.json`
- 检测 watch_dirs[0] 是否与 archive_dir 一致
- 一致 → 自动合并
- 不一致 → 警告，保留 kb-graph-config.json 不动（双 config 共存，kb_graph 仍可工作但 kb_query 会用 archive_dir）

### 7.4 风险：路径不一致的处理

当前 KB Graph 的 watch_dirs 与 Link Archivist 的 archive_dir 不一致。这是一个**已存在的配置错误**。合并方案必须处理这个边界。

**设计决策**：
- Link Archivist 启动时检测两个目录
- 提示用户："检测到你之前配置了不同的 watch_dirs，是否迁移？"
- 选项：
  1. 合并到 archive_dir（推荐，522 个文件已有索引）
  2. 保留独立 watch_dirs（kb_index.watch_dirs 字段，可选）
  3. 忽略 KB Graph 旧配置

**简化决策**：v2.0.0 强制以 archive_dir 为准。watch_dirs 字段保留但不推荐使用（标记 deprecated）。
...
1. **KB Graph 旧 Skill 的处理**：立即删除 vs 保留 6 个月 deprecated？
2. **LLM 重新编译触发条件**：是否在 Phase 3 强制要求 LLM 输出 frontmatter 字段？还是允许 frontmatter 缺失时再补调 LLM？
3. **跨目录索引**：是否保留 kb_index.watch_dirs 字段支持多目录？还是强制以 archive_dir 为准？
4. **语义搜索**：默认禁用还是启用？需要 OPENAI_API_KEY 是否过重？
5. **Phase 3 LLM 输出要求**：是否需要在 SKILL.md 中提供标准 prompt 模板，确保 LLM 输出格式稳定？

---

_方案文档 v1.0 草案，待 Factory Reviewer 审核。_

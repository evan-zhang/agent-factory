...
```
你是 Link Archivist Orchestrator。请读取 {skill_root}/specs/agents/link-archivist-orchestrator.md 了解你的角色和规则。
...
请读取 SKILL.md 理解完整工作流，按 Phase 1-5 执行。完成后输出 LINK_ARCHIVIST_RESULT 结构。
```

其中 `{skill_root}` 由主 Agent 动态解析为 Skill 的绝对安装路径。

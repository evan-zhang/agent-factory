...
2. **抓取内容** → r.jina.ai（通用）/ yt-dlp（YouTube）/ 外部文件解析
   - **抖音视频**（v.douyin.com / douyin.com）：先加载 `douyin-video-analysis` skill（`projects/2604171/douyin-video-analysis/`），调用 `scripts/douyin_process.py` 完成视频下载、音频提取、报告生成，然后基于其输出继续调研流程

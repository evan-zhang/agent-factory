...
- 最终路径：`{video_archive_dir}/YYYY/MM/DD/{archive_id}.mp4`
- 进度提示：`💾 视频已归档：<视频路径>`

**Doc Viewer 上传**（可选，在归档完成后）：
- 条件：HTML 或 Markdown 文件归档成功
- 调用 Doc Viewer API 上传归档文件
- 命令：`curl -s -X POST http://doc.20100706.xyz/upload -F "file=@<归档文件路径>;filename=<原始文件名>"`
- 返回 JSON 中的 `url` 字段即为在线预览链接
- 进度提示：`🔗 预览链接：<url>`

**完成**：归档路径 + 预览链接确认，流程结束。
**用户可干预**：用户可指定归档目录、修改文件名、取消 Obsidian 同步。
...
**本 Skill 负责**：抓取 → 判断 → 调研 → 生成报告 → 归档本地 → 同步 Obsidian → 上传 Doc Viewer 生成预览链接

**不负责**：渠道发送（Agent 自行决定）、文件解析（PDF/Word/PPT/图片）、知识索引管理（AF-20260413-003）

**依赖服务**：Doc Viewer（`http://doc.20100706.xyz`）— 文件上传与在线预览，Skill 内置调用，无需额外配置。

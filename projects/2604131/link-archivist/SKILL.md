...
2. **抓取内容** → r.jina.ai（通用）/ yt-dlp（YouTube）/ 外部文件解析
   - **抖音视频**（v.douyin.com / douyin.com）：
     1. 检查 `douyin-video-analysis` skill 是否已安装（检查 `~/.openclaw/skills/douyin-video-analysis/` 或 `projects/2604171/douyin-video-analysis/` 目录）
     2. 未安装 → 提示用户安装：
        > 检测到抖音链接，但未安装抖音视频分析 Skill。
        > 请先安装：`git clone --depth 1 --sparse https://github.com/evan-zhang/agent-factory.git && cd agent-factory && git sparse-checkout set projects/2604171/douyin-video-analysis`
     3. 已安装 → 调用 `douyin-video-analysis` 的 `scripts/douyin_process.py` 完成视频下载、音频提取、报告生成，然后基于其输出继续调研流程

...
2. **通知用户**：立即回复，告知已安排后台处理及预计时间
   ...
   - short 模式：1-2 分钟
   - 含视频下载：额外 +1-2 分钟
3. **spawn orchestrator**：`sessions_spawn` 创建 depth 1 orchestrator
4. **等待完成**：orchestrator 完成后自动 announce
5. **汇报结果**：主 Agent 收到 announce，用正常对话语气向用户报告结果

**多链接并行**：收到多个链接时，每个链接 spawn 一个独立的 orchestrator（depth 1），互不阻塞。全局最多 8 个并发 sub-agent。

### 主 Agent 初始回复模板

收到链接后，**立即**回复用户（spawn 之前）：

单个链接：
> 收到链接，我来帮你处理。
> 计划：抓取内容 → 判断模式 → 调研生成报告 → 验证 → 归档
> 预计 3-5 分钟，处理完通知你。你可以继续发其他消息。

多个链接：
> 收到 {N} 个链接，已安排并行处理。
> 每个链接：抓取 → 判断 → 调研 → 验证 → 归档
> 预计 3-5 分钟全部完成，逐个通知结果。

### 主 Agent spawn 模板

主 Agent spawn 时，task 参数示例：
```
你是 Link Archivist Orchestrator。请读取 {baseDir}/specs/agents/link-archivist-orchestrator.md 了解你的角色和规则。

参数：
- skill_root: {baseDir}
- url: {url}
- mode_hint: {mode_hint 或 "自动判断"}
- extra_context: {extra_context 或 "无"}

请读取 {baseDir}/SKILL.md 理解完整工作流，按 Phase 1-5 执行。完成后输出 LINK_ARCHIVIST_RESULT 结构。
```

其中 `{baseDir}` 是 OpenClaw 自动解析的 Skill 安装目录绝对路径，无需手动配置。
...
> **路径约定**：以下所有 `scripts/xxx.py` 和 `references/xxx.md` 都是相对于 `skill_root` 的路径。执行时必须拼接为绝对路径：`{skill_root}/scripts/xxx.py`。

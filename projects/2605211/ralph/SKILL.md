...
name: ralph
description: |
  Ralph Loop — CLI 持续编程工具（非 OpenClaw Skill/插件）。
  基于 Geoffrey Huntley 的 Ralph Loop 模式，通过 bash 循环驱动 AI CLI 执行器
  （Claude Code / Codex）完成多阶段、长流程的编程任务。
  三种模式：
  ...
  安装：clone 仓库，直接运行 scripts/ralph-loop.sh，不依赖 OpenClaw。
metadata:
  version: "3.1.1"
...
# Ralph Loop — CLI 持续编程工具

> **定位**：Ralph Loop 是一个独立的命令行工具，不是 OpenClaw 插件或 Skill。
> 由人决定什么时候使用，不会被 agent 自动发现或调用。
> 与 OpenClaw Goal Mode 互不依赖，各自独立使用。
...
Ralph Loop 是独立的 CLI 工具，不需要安装到 OpenClaw skills 目录。clone 仓库后直接使用。

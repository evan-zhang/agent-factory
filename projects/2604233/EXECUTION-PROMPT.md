...
=== Phase 2：上传至 Cloud File Store ===

完成全部 7 个模块采集后，将本地结果文件上传至 Cloud File Store，供外部系统读取。

#### 前置条件检查

1. 确认环境变量 `CLOUD_FILE_STORE_KEY` 已注入：
   - 运行：`echo $CLOUD_FILE_STORE_KEY`
   - 如未注入，跳过 Phase 2，在报告中标注「Cloud File Store 上传跳过：缺少 API Key」

2. 确认 Cloud File Store 服务可达：
   - 运行：`curl -s http://103.236.97.248:8900/health`
   - 如返回非 200 或超时，跳过 Phase 2，在报告中标注「Cloud File Store 上传跳过：服务不可达」

#### 上传规则

对以下 7 个目录中的所有 Markdown 文件逐一上传（覆盖写入，只保留最新）：

```
./七大政策/{目标城市}/01-异地就医/
./七大政策/{目标城市}/02-异地生育报销/
./七大政策/{目标城市}/03-公积金异地购房贷款/
./七大政策/{目标城市}/04-购房资格/
./七大政策/{目标城市}/05-车牌摇号/
./七大政策/{目标城市}/06-子女上学/
./七大政策/{目标城市}/07-落户/
...
每个目录中上传存在的文件（01-指标汇总表.md / 02-来源明细表.md / 03-缺口与待补充.md）。
...
```bash
curl -s -X PUT "http://103.236.97.248:8900/skills/seven-policy-collector/upload" \
...
示例（北京，异地就医）：
```bash
# 本地路径：./七大政策/北京/01-异地就医/01-指标汇总表.md
# 上传路径：七大政策/北京/01-异地就医/01-指标汇总表.md

curl -s -X PUT "http://103.236.97.248:8900/skills/seven-policy-collector/upload" \
  -H "X-API-Key: $CLOUD_FILE_STORE_KEY" \
  -F "file=@./七大政策/北京/01-异地就医/01-指标汇总表.md" \
  -F "path=七大政策/北京/01-异地就医/01-指标汇总表.md"
```

#### 上传验证

每个文件上传后检查返回值：
- 返回 JSON 包含 `"is_dir": false` 且有 `"size"` 字段 → 上传成功
- 其他情况 → 记录为上传失败，继续上传下一个文件（不中止整体流程）
...
6. Cloud File Store 上传结果：成功/失败文件数（如：成功上传 21/21），失败文件列表（如有）
7. 云端访问路径：`skills/seven-policy-collector/七大政策/{目标城市}/`

...
支持多 profile 通用渲染（A-1/A-5/A-7 等）
...
示例：python3 render.py /path/to/MB-001-Mage-Biologics mckinsey-navy /tmp/report.html A-1
      python3 render.py /path/to/MB-001-Mage-Biologics mckinsey-navy /tmp/report.html A-5
      python3 render.py /path/to/MB-001-Mage-Biologics mckinsey-navy /tmp/report.html A-7
...
def load_profile(profile_code):
    """加载 profile 配置文件"""
    script_dir = Path(__file__).parent
    profile_path = script_dir / "profiles" / f"{profile_code}.json"

    if not profile_path.exists():
        print(f"⚠ Profile 不存在: {profile_path}，使用通用配置")
        return None

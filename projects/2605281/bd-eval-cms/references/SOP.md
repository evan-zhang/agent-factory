...
- 报告末尾参考文献表按前缀分组排序，格式：`[P1-001] 标题 — URL（抓取时间）`、`[G1-003] 标题 — URL（抓取时间）`
- 报告正文中所有数据点必须有对应的前缀编号引用
...
| 7 | 来源标注 | 所有数据点有 [{前缀}-XXX] 引用，无”外网核查”等无编号标注 | PASS/FAIL |
| 8 | 财务缩写 | 首次出现标注中文全称 | PASS/FAIL |
| 9 | 参考文献表 | 末尾有完整参考文献表，按前缀分组，覆盖所有正文引用 | PASS/FAIL |

...
> **注意**：脚本实际用 `python3 -c` 解析 config.yaml（不依赖 `yq`），AppKey 从 `.secrets/kb_appkey` 文件读取（不是环境变量）。以下命令是等价的手动写法，正式跑用 `sync-to-knowledge-base.sh`。

```bash
# 脚本内部实际读法（无需 yq）：
#   python3 -c "import re; ..." 解析 config.yaml
# AppKey：cat bd-eval-cms/.secrets/kb_appkey

PROJECT_ID="2060176831872499713"  # config.yaml knowledgeBase.projectId
DEFAULT_STYLE="a1"             # config.yaml reportRenderer.defaultStyle
DEFAULT_COLOR="mckinsey-navy"   # config.yaml reportRenderer.defaultColorTheme
APP_KEY="$(cat bd-eval-cms/.secrets/kb_appkey | tr -d '[:space:]')"
```
...
**关键约束**：
- AppKey 走 `bd-eval-cms/.secrets/kb_appkey` 文件（v0.10.3 起，不入仓），**禁止硬编码**
...
- appKey：从 `.secrets/kb_appkey` 文件读取（v0.10.3 起，不依赖环境变量）
- projectId：从 `bd-eval-cms/config.yaml` 的 `knowledgeBase.projectId` 读取（业务固定值）

> 配置信息字段：v0.10.3 起 appKey 从 `.secrets/kb_appkey` 文件读取，projectId 从 `bd-eval-cms/config.yaml` 的 `knowledgeBase.projectId` 读取（业务固定值）
